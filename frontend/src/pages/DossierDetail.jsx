...
export default function DossierDetail() {
  ...
  if (!isLoaded || loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-56px)] text-text-secondary">
        ...
        <span className="font-mono text-xs">Retrieving dossier details...</span>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="max-w-md mx-auto px-6 mt-16 text-center space-y-4">
        <AlertTriangle className="text-accent-red mx-auto" size={40} />
        <h3 className="font-display font-bold text-lg text-text-primary uppercase">Dossier Unavailable</h3>
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 mt-8 space-y-6 pb-24">
      {/* Header controls */}
      <div className="flex items-center justify-between border-b border-border-subtle/50 pb-4 font-mono text-xs">
        <Link 
          to="/history" 
